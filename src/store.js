...
export default createStore({
  ...
  mutations: {
    add(state, payload) {
      const card = state.cards[payload.idx]
      if (state.basket.indexOf(card) === -1) {
        state.basketCount++
      }
      state.basket.push(card)
      state.total += card.price
    },
    remove(state, payload) {
      const card = state.cards[payload.idx]
      if (state.basket.indexOf(card) >= 0) {
        state.total -= card.price
        state.basket.splice(state.basket.findIndex(el => el.id === payload.id), 1)
        if (state.basket.indexOf(card) === -1) {
          state.basketCount--
        }
      }
